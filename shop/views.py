...
# I have created this file - Rohith
...
def index(request):
    ...
    rohith = {'allProds': allProds}
    return render(request, 'shop/index.html', rohith)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    rohith = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        rohith = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', rohith)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            rohith_dict['CHECKSUMHASH'] = Checksum.generateChecksum(rohith_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'rohith_dict': rohith_dict})
...
def handlerequest(request):
    ...
    verify = Checksum.verifyChecksum(response_dict, MERCHANT_KEY, checksum)
    if verify:
